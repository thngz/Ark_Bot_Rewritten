...
class Arkbot:
    ...
    def create_table(self):   
                ...
                self.cursor.execute("CREATE TABLE IF NOT EXISTS exams (location VARCHAR(255) PRIMARY KEY, first VARCHAR(255), second VARCHAR(255), third VARCHAR(255), link VARCHAR(255))")
    ...
    def scrape(self):
            ...
            try:
                for tr in self.soup.findAll('tr')[1:]:
                        ...
                        show_changed_sql = "MYSQL-AFFECTED-ROWS"
                        values = (location, first_available, second_available, third_available, 'eteenindus.mnt.ee/main.jsf')
            ...
            if len(self.diffs) == 0:
                ...
                try:
                    for diff in self.diffs:
                        tweet = "\n".join(map(str, diff)).replace("{", "").replace("}", "").replace("'", "").replace(",", "\n")
                        if tweet:            
                                print(tweet)
                                print("Sending tweet")
